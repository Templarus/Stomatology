 ...
 * @author TemplaRus
 */
public class Icon_PrevichTooth extends javax.swing.JPanel
{
     ...
     *
     * @param root_count - кол-во корней
     * @param tooth_type - тип id_categ_rab из таблицы S_Pervich
     * @param isScaled - увеличенный объект для popup
     * @param tooth_number - номер зуба
     * @param forPopup - для отображения в popup
     */
    public Icon_PrevichTooth(int root_count, int tooth_type, int tooth_number, boolean isScaled, boolean forPopup)
    {
        ...
        this.zub = Find.mf.cl.getPervichObsled().get(MainForm.idObsled).getPervichObsledStr().get(tooth_number);
    ...
    private void Tooth_full_type0()//рисуем зуб корнями вверх - статус внизу ЗУБ С 2 центральными частями 
    {
    ...
    private void Tooth_full_type1()//рисуем зуб корнями вниз= статус вверху  ЗУБ С 2 центральными частями 
    {
    ...
    private void Tooth_full_type2()//рисуем зуб корнями вверх - статус внизу ЗУБ С ЦЕЛОЙ центральной частью 
    {
        figure0 = new int[][]{{20, 10, 4, 4, 10, 20, 18}, {20, 10, 20, 36, 46, 36, 28}, {0, 48}};
        figure1 = dummyfigure;
        figure2 = dummyfigure;
    ...
    private void Tooth_full_type3()//рисуем зуб корнями вниз= статус вверху  ЗУБ С ЦЕЛОЙ центральной частью
    {
        figure0 = new int[][]{{20, 10, 4, 4, 10, 20, 18}, {20, 10, 20, 36, 46, 36, 28}, {0, 0}};//координаты икс,координаты игрек,приращения ,цвет
        figure1 = dummyfigure;
        figure2 = dummyfigure;
        ...
        switch (root_count) {
            case 1:
                if (center_splited) {
                    ...
                    figures = new int[][][]{figure0, figure1, figure2, figure3, figure4, figure5, figure6, dummyfigure, dummyfigure, figure9};
                }
                break;
   ...
   if (tooth_number < 16) {
    ...
    private void Draw_polygons()
    {
        ...
        for (int[][] figure : figures)//Для каждой фигуры в из массива фигур
        {
            ...
            ColorsReady[y] = new Color(zub.getStatGranColor().get(y).getColor()[0], zub.getStatGranColor().get(y).getColor()[1], zub.getStatGranColor().get(y).getColor()[2]);//Грани зуба привязываем цвет
        ...
        for (int i = 6; i < 12; i++) {
            if (zub.getStatGranColor().get(i).getIdPervich() != -1) {

                switch (zub.getStatGranColor().get(i).getIdPervich()) {
                    case 42:
                        Rooth_Icon_type0(i);
                        break;
                    case 45:
